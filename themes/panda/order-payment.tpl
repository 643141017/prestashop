			...
			<div id="HOOK_TOP_PAYMENT">{$HOOK_TOP_PAYMENT}</div>
				{if $HOOK_PAYMENT}
					{if !$opc}
						<div id="order-detail-content" class="table_block">
							<table id="cart_summary" class="table table-bordered resp_table">
								<thead>
									<tr>
										<th class="cart_product first_item">{l s='Product'}</th>
										<th class="cart_description item">{l s='Description'}</th>
										{if $PS_STOCK_MANAGEMENT}
											<th class="cart_availability item text-right">{l s='Availability'}</th>
										{/if}
										<th class="cart_unit item text-center">{l s='Unit price'}</th>
										<th class="cart_quantity item text-right">{l s='Qty'}</th>
										<th class="cart_total last_item text-center">{l s='Total'}</th>
									</tr>
								</thead>
								<tfoot>
									{if $use_taxes}
									...
									<tr class="cart_total_price">
										{if $voucherAllowed}
											<td colspan="2" class="cart_voucher">
				...
				{if !$opc}
					<p class="cart_navigation clearfix">
						<a href="{$link->getPageLink('order', true, NULL, "step=2")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default pull-left">
