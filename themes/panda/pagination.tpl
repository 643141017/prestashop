...
{if !isset($no_follow_text)}
	{if isset($no_follow) AND $no_follow}
		{assign var='no_follow_text' value='rel="nofollow"' scope=parent}
	{else}
		{assign var='no_follow_text' value='' scope=parent}
	{/if}
{/if}

{if isset($p) AND $p}
	{if !isset($requestPage) || !isset($requestNb)}
		{if isset($smarty.get.id_category) && $smarty.get.id_category && isset($category)}
			{if !isset($current_url)}
				{assign var='requestPage' value=$link->getPaginationLink('category', $category, false, false, true, false) scope=parent}
			{else}
				{assign var='requestPage' value=$current_url scope=parent}
			{/if}
			{assign var='requestNb' value=$link->getPaginationLink('category', $category, true, false, false, true) scope=parent}
		{elseif isset($smarty.get.id_manufacturer) && $smarty.get.id_manufacturer && isset($manufacturer)}
			{assign var='requestPage' value=$link->getPaginationLink('manufacturer', $manufacturer, false, false, true, false) scope=parent}
			{assign var='requestNb' value=$link->getPaginationLink('manufacturer', $manufacturer, true, false, false, true) scope=parent}
		{elseif isset($smarty.get.id_supplier) && $smarty.get.id_supplier && isset($supplier)}
			{assign var='requestPage' value=$link->getPaginationLink('supplier', $supplier, false, false, true, false) scope=parent}
			{assign var='requestNb' value=$link->getPaginationLink('supplier', $supplier, true, false, false, true) scope=parent}
		{else}
			{if !isset($current_url)}
				{assign var='requestPage' value=$link->getPaginationLink(false, false, false, false, true, false) scope=parent}
			{else}
				{assign var='requestPage' value=$current_url scope=parent}
			{/if}
			{assign var='requestNb' value=$link->getPaginationLink(false, false, true, false, false, true) scope=parent}
		{/if}
	{/if}
	<!-- Pagination -->
		...
		{if $start!=$stop}
			<ul class="pagination clearfix li_fl">
				{if $p != 1}
					...
					<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_previous"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}" title="{l s='Previous'}" rel="prev"><i class="icon-left-open-3"></i></a></li>
				{else}
					<li id="pagination_previous{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_previous"><span><i class="icon-left-open-3"></i></span></li>
				{/if}
				{if isset($paginationId)}
					{if $start==3}
				...
				{if $pages_nb > 1 AND $p != $pages_nb}
					...
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="pagination_next"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}" title="{l s='Next'}" rel="next"><i class="icon-right-open-3"></i></a></li>
				{else}
					<li id="pagination_next{if isset($paginationId)}_{$paginationId}{/if}" class="disabled pagination_next"><span><i class="icon-right-open-3"></i></span></li>
				{/if}
			</ul>
		{/if}
	    {if $nb_products > $products_per_page && $start!=$stop}
	    	{if Configuration::get('STSN_CATEGORY_SHOW_ALL_BTN')}
