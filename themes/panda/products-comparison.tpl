					...
					<td class="ajax_block_product comparison_infos product-block product-{$product->id}">
						<div class="product-image-block">
							...
							{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
								{if $product->on_sale}
									<span class="on_sale"><i>{l s='Sale'}</i></span>
								{/if}
							{/if}
								...
								{if $product->on_sale}
									{elseif $product->specificPrice AND $product->specificPrice.reduction}
										<div class="product_discount">
											<span class="reduced-price sm_lable">{l s='Reduced price!'}</span>
										</div>
									{/if}
							...
							<div class="clearfix">
								{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
									{if ($product->quantity > 0 OR $product->allow_oosp)}
										<a class="ajax_add_to_cart_button btn btn-default pull-left" data-id-product="{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" title="{l s='Add to cart'}">
									...
									{/if}
								{/if}
								<a class="cmp_remove pull-right" href="{$link->getPageLink('products-comparison', true)|escape:'html':'UTF-8'}" title="{l s='Remove'}" data-id-product="{$product->id}"><i class="icon-cancel icon-small"></i></a>
							</div>
