...
<div class="box box-small clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<input type="hidden" value="" name="submitReorder"/>
				...
				<a href="#" onclick="$(this).closest('form').submit(); return false;" class="btn btn-default pull-right"><span>{l s='Reorder'}<i class="icon-chevron-right right"></i></span></a>
			<p class="dark">
...
<div id="order-detail-content" class="table_block">
	<table class="table table-bordered resp_table">
		<thead>
			<tr>
		...
		<tfoot>
			...
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
					<span class="price price-wrapping">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
				</td>
			</tr>
			{/if}
			<tr class="item">
				...
				<td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
					<span class="price price-shipping">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
				</td>
			</tr>
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						{if $order->hasProductReturned()}
							<td data-title="{l s='Returned'}">
								<div class="mobile_table_content">
								{$product['qty_returned']}
								</div>
							</td>
						{/if}
						<td data-title="{l s='Unit price'}">
							<div class="mobile_table_content">
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="return_quantity" data-title="{l s='Quantity'}">
							<div class="mobile_table_content">
						...
						{/if}
						<td class="price" data-title="{l s='Unit price'}">
							<div class="mobile_table_content">
							<label for="cb_{$product.id_order_detail|intval}">
							{if $group_use_tax}
								{convertPriceWithCurrency price=$product.unit_price_tax_incl currency=$currency}
						...
						<td class="price" data-title="{l s='Total price'}">
							<div class="mobile_table_content">
							<label for="cb_{$product.id_order_detail|intval}">
							{if $group_use_tax}
								{convertPriceWithCurrency price=$product.total_price_tax_incl currency=$currency}
...
{if $return_allowed}
	<div id="returnOrderMessage">
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-default">{l s='Make an RMA slip'}</button>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="btn btn-default">{l s='Send'}</button>
		</div>
	</form>
{else}
