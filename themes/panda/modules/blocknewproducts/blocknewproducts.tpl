...
<div id="new-products_block_right" class="block column_block products_block">
	<h3 class="title_block">
    	<a href="{$link->getPageLink('new-products')|escape:'html'}" title="{l s='All new products' mod='blocknewproducts'}">{l s='New products' mod='blocknewproducts'}</a>
    </h3>
            ...
            <ul class="pro_column_list">
                {foreach from=$new_products item=newproduct name=myLoop}
                    <li class="pro_column_box clearfix">
                        <a class="pro_column_left products-block-image" href="{$newproduct.link|escape:'html'}" title="{$newproduct.legend|escape:html:'UTF-8'}"><img class="replace-2x img-responsive" src="{$link->getImageLink($newproduct.link_rewrite, $newproduct.id_image, 'small_default')|escape:'html'}" alt="{$newproduct.name|escape:html:'UTF-8'}" width="{$smarty.capture.small_default_width}" height="{$smarty.capture.small_default_height}" /></a>
