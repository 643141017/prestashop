...
<section id="crossselling-products_block_center" class="block products_block section">
    <h3 class="title_block {if Configuration::get('STSN_CS_TITLE')} title_block_center {/if}">
        <span>
    ...
    {assign var='cs_direction_nav' value=Configuration::get('STSN_CS_DIRECTION_NAV')}
	<div id="crossselling-itemslider" class="products_slider">
            <div class="slides remove_after_init {if $cs_direction_nav>1} owl-navigation-lr {if $cs_direction_nav==4} owl-navigation-circle {else} owl-navigation-rectangle {/if} {elseif $cs_direction_nav==1} owl-navigation-tr{/if}">
                    ...
                    <div class="ajax_block_product {if $product@first}first_item{elseif $product@last}last_item{else}item{/if}">
                        ...
                        {capture name="pro_a_cart"}
                            {if !$PS_CATALOG_MODE && ($product.allow_oosp || $product.quantity > 0)}
                                <a class="ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}&amp;add")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='crossselling'}" data-id-product="{$product.id_product|intval}"><div><i class="icon-glyph icon_btn icon-small icon-mar-lr2"></i><span>{l s='Add to cart' mod='crossselling'}</span></div></a>
                            {/if}
                        {/capture}
