...
<div id="layered_block_left" class="block column_block">
	<h3 class="title_block"><span>{l s='Catalog' mod='blocklayered'}</span></h3>
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="javascript:;" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"></a>
					...
					{if isset($filter.values)}
                        ...
                        <div class="layered_subtitle_heading clearfix">
                            ...
                            <span class="layered_close">
                            	<a href="javascript:;" data-rel="ul_layered_{$filter.type}_{$filter.id_key}"></a>
                            </span>
						</div>
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if} clearfix">
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} filter_lg">
