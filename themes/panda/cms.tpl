...
{elseif isset($cms_category)}
	<div class="block-cms">
		 <h1 class="heading page-heading"><a href="{if $cms_category->id eq 1}{$base_dir}{else}{$link->getCMSCategoryLink($cms_category->id, $cms_category->link_rewrite)}{/if}">{$cms_category->name|escape:'html':'UTF-8'}</a></h1>
		{if isset($cms_category->description) && $cms_category->description}
			<div class="block">{$cms_category->description|escape:'html':'UTF-8'}</div>
		{/if}
		{if isset($sub_category) && !empty($sub_category)}	
			<h3 class="heading page-heading">{l s='List of sub categories in %s:' sprintf=$cms_category->name}</h3>
			<ul class="bullet">
		...
		<h3 class="heading page-heading">{l s='List of pages in %s:' sprintf=$cms_category->name}</h3>
			<ul class="bullet">
