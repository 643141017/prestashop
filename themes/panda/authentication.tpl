...
{if !isset($email_create)}
	...
	{/if}-->
	{capture name="oneallsociallogin"}{hook h='displayAnywhere' mod='oneallsociallogin' caller='oneallsociallogin'}{/capture}
	<div class="row">
		<div class="col-xs-12 col-sm-6">
			...
			<section class="block">
				...
				<div class="form_content {if isset($smarty.capture.oneallsociallogin) && $smarty.capture.oneallsociallogin|trim} has_social_login {/if}">
            	...
            	{if isset($smarty.capture.oneallsociallogin) && $smarty.capture.oneallsociallogin|trim}
            		{$smarty.capture.oneallsociallogin}
            	{/if}
				</div>
				<div class="submit">
					{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
					<button class="btn btn-default btn_primary" type="submit" id="SubmitCreate" name="SubmitCreate">
						<i class="icon-user left icon-small"></i>
		...
		<div class="col-xs-12 col-sm-6">
			...
			<section class="block">
				...
				<div class="submit">
					{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'html':'UTF-8'}" />{/if}
					<button type="submit" id="SubmitLogin" name="SubmitLogin" class="btn btn-default btn_primary">
						<i class="icon-lock left icon-small"></i>
						{l s='Sign in'}
					</button>
				</div>
			</section>
		...
		<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="std clearfix">
			<div class="box">
				<div id="opc_account_form" style="display: block; ">
					<h3 class="page-subheading">{l s='Instant checkout'}</h3>
					<!-- Account -->
					...
					<div class="form-stru gender-line">
					...
					{if $dniExist eq false}
						<div class="required form-group dni">
							<label for="dni">{l s='Identification number'} <sup>*</sup></label>
							<input type="text" class="text form-control" name="dni_invoice" id="dni_invoice" value="{if isset($smarty.post.dni) && $smarty.post.dni}{$smarty.post.dni}{/if}" />
							<span class="form_info">{l s='DNI / NIF / NIE'}</span>
						</div>
					{/if}
					...
					<div id="opc_invoice_address"  class="unvisible">
						...
						{if $field_name eq "company"}
						<div class="form-group">
							<label for="company_invoice">{l s='Company'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
						...
						<div id="vat_number_block_invoice" style="display:none;">
							<div class="form-group">
								<label for="vat_number_invoice">{l s='VAT number'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
						...
						{elseif $field_name eq "address2"}
						<div class="form-group is_customer_param">
							<label for="address2_invoice">{l s='Address (Line 2)'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
			...
			<p class="cart_navigation required submit clearfix">
				...
				<button type="submit" class="btn btn-default" name="submitGuestAccount" id="submitGuestAccount">
		...
		<div class="account_creation">
			...
			{if isset($newsletter) && $newsletter}
				<div class="checkbox">
					...
					{if isset($field_required) && array_key_exists('newsletter', $field_required)}
						<sup> *</sup>
					{/if}
				</div>
			{/if}
			{if isset($optin) && $optin}
				<div class="checkbox">
					...
					{if isset($field_required) && array_key_exists('optin', $field_required)}
						<sup> *</sup>
					{/if}
				</div>
			{/if}
			...
			<div class="account_creation">
				...
				{foreach from=$dlv_all_fields item=field_name}
					...
					{elseif $field_name eq "vat_number"}
						<div id="vat_number" style="display:none;">
							<p class="form-group">
								<label for="vat_number">{l s='VAT number'}{if isset($required_fields) && in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
