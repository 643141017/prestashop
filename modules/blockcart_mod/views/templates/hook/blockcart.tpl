...
*}
<!-- MODULE Block cart -->
	<div class="shopping_cart {if $PS_CATALOG_MODE} header_user_catalog{/if} top_bar_item {if $block_cart_style} shopping_cart_style_1 {/if} hidden-sm hidden-xs">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" id="shopping_cart" title="{l s='View my shopping cart' mod='blockcart_mod'}" rel="nofollow" class="header_item">
			<div class="ajax_cart_bag">
				<span class="ajax_cart_quantity amount_circle {if $cart_qties > 9} dozens {/if}">{$cart_qties}</span>
				<span class="ajax_cart_bg_handle"></span>
			</div>
			<span class="cart_text">{l s='Shopping cart' mod='blockcart_mod'}</span>
			<span class="ajax_cart_quantity">{$cart_qties}</span>
			<span class="ajax_cart_product_txt">{l s='item(s)' mod='blockcart_mod'}</span>
			<span class="ajax_cart_split">{l s='-' mod='blockcart_mod'}</span>
		...
		{if !$PS_CATALOG_MODE && $hover_display_cp}
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="clearfix {if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										...
										<span class="remove_link">
											{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
												<a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart_mod'}"><i class="icon-cancel"></i></a>
											{/if}
										</span>
										<span class="price">
...
{if !$PS_CATALOG_MODE && $addtocart_animation==1 && $active_overlay == 1}
	<div id="layer_cart" class="layer_box">
		<div class="layer_inner_box">
			<div class="layer_product clearfix mar_b10">
				<span class="cross" title="{l s='Close window' mod='blockcart_mod'}"></span>
				...
				<div class="layer_product_info">
					...
					<div id="layer_cart_product_quantity_wrap">
						<span class="layer_cart_label">{l s='Quantity' mod='blockcart_mod'}</span>
						<span id="layer_cart_product_quantity"></span>
					</div>
					<div id="layer_cart_product_price_wrap">
						<span class="layer_cart_label">{l s='Total' mod='blockcart_mod'}</span>
						<span id="layer_cart_product_price"></span>
					</div>
				</div>
			</div>
			<div id="pro_added_success" class="success">{l s='Product successfully added to your shopping cart' mod='blockcart_mod'}</div>
			<div class="layer_details">
				<div class="layer_cart_sum">
					<!-- Plural Case [both cases are needed because page may be updated in Javascript] -->
					<span class="ajax_cart_product_txt_s {if $cart_qties < 2} unvisible{/if}">
						{l s='There are [1]%d[/1] products in your cart.' mod='blockcart_mod' sprintf=[$cart_qties] tags=['<span class="ajax_cart_quantity">']}
					</span>
					<!-- Singular Case [both cases are needed because page may be updated in Javascript] -->
					<span class="ajax_cart_product_txt {if $cart_qties > 1} unvisible{/if}">
						{l s='There is 1 product in your cart.' mod='blockcart_mod'}
					</span>
				</div>
				<div id="layer_cart_ajax_block_products_total" class="layer_cart_row">
					<span class="layer_cart_label">
						...
						{else}
							{l s='(tax incl.)' mod='blockcart_mod'}
						{/if}
					</span>
				...
				<div id="layer_cart_ajax_cart_shipping_cost" class="layer_cart_row hidden">
					<span class="layer_cart_label{if $shipping_cost_float == 0 && (!isset($cart->id_address_delivery) || !$cart->id_address_delivery)} unvisible{/if}">
						{l s='Total shipping' mod='blockcart_mod'}&nbsp;{if $priceDisplay == 1}{l s='(tax excl.)' mod='blockcart_mod'}{else}{l s='(tax incl.)' mod='blockcart_mod'}{/if}
					</span>
				...
				{if $show_tax && isset($tax_cost)}
					<div id="layer_cart_ajax_cart_tax_cost" class="layer_cart_row hidden">
						<span class="layer_cart_label">{l s='Tax' mod='blockcart_mod'}</span>
						<span class="price cart_block_tax_cost ajax_cart_tax_cost">{$tax_cost}</span>
					</div>
				{/if}
				...
				<div class="button-container row">
					<div class="col-xxs-12 col-xs-6">
						<span class="continue btn btn-default btn-bootstrap" title="{l s='Continue shopping' mod='blockcart_mod'}">
							{l s='Continue shopping' mod='blockcart_mod'}
						</span>	
					</div>
					<div class="col-xxs-12 col-xs-6">
						<a class="btn btn-default btn-bootstrap" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart_mod'}" rel="nofollow">
							{l s='Check out' mod='blockcart_mod'}
						</a>	
					</div>
				</div>
			</div>
...
{addJsDef cart_size_width=$cartSize.width|intval}
{addJsDef cart_size_height=$cartSize.height|intval}
{addJsDef hasDeliveryAddress=(isset($cart->id_address_delivery) && $cart->id_address_delivery)}

{addJsDefL name=customizationIdMessage}{l s='Customization #' mod='blockcart_mod' js=1}{/addJsDefL}
{addJsDefL name=removingLinkText}{l s='remove this product from my cart' mod='blockcart_mod' js=1}{/addJsDefL}
{addJsDefL name=freeShippingTranslation}{l s='Free shipping!' mod='blockcart_mod' js=1}{/addJsDefL}
{addJsDefL name=freeProductTranslation}{l s='Free!' mod='blockcart_mod' js=1}{/addJsDefL}
{addJsDefL name=delete_txt}{l s='Delete' mod='blockcart_mod' js=1}{/addJsDefL}
