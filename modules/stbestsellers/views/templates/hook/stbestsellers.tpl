...
{if isset($homeverybottom) && $homeverybottom}<div class="wide_container {if $hide_mob} hidden-xs {/if} block"><div class="container">{/if}
<section id="best-sellers_block_center_{$hook_hash}{$smarty.capture.column_slider}" class="best-sellers_block_center{$smarty.capture.column_slider} {if !isset($homeverybottom) || !$homeverybottom} block {/if} {if isset($column_slider) && $column_slider} column_block {/if} products_block {if !!isset($column_slider) || !$column_slider} section {/if} {if $hide_mob} hidden-xs {/if} {if $display_as_grid==1} display_as_grid {elseif $display_as_grid==2} display_as_simple {/if}">
	<h3 class="title_block {if (!isset($column_slider) || !$column_slider) && $title_position} title_block_center {/if}"><a href="{$link->getPageLink('best-sales')|escape:'html':'UTF-8'}" title="{l s='Top sellers' mod='stbestsellers'}">{l s='Top sellers' mod='stbestsellers'}</a></h3>
    <script type="text/javascript">
    //<![CDATA[
    var best_itemslider_options{$smarty.capture.column_slider};
    //]]>
    </script>
	{if isset($products) AND $products}
    {if !$display_as_grid || ($display_as_grid && isset($column_slider) && $column_slider)}
    <div id="bestsellers-itemslider_{$hook_hash}{$smarty.capture.column_slider}" class="bestsellers-itemslider{$smarty.capture.column_slider} products_slider block_content">
    ...
    {literal}
    jQuery(function($) {
        var owl = $("#bestsellers-itemslider_{/literal}{$hook_hash}{$smarty.capture.column_slider}{literal} .slides");
        owl.owlCarousel({
            {/literal}
    ...
    {/if}
    {else}
        <div class="block_content"><div class="warning">{l s='No best sellers at this time' mod='stbestsellers'}</div></div>
    {/if}
</section>
