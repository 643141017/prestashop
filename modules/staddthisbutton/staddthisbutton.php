...
class StAddThisButton extends Module
{
	...
	public function install()
	{
		if (!parent::install() 
            ...
            || !Configuration::updateValue('ST_ADDTHIS_STYLE', 0)
    ...
    public function hookDisplayHeader()
    {
        ...
        if($page_name == 'module-stblog-article' && $id_st_blog = Tools::getValue('id_blog'))
        {
            ...
            $cover = StBlogImageClass::getCoverImage($id_st_blog, $id_lang, $blog->type);
            if ($cover)
                $cover = StBlogImageClass::getImageLinks($cover,1,$blog->type);
            $this->smarty->assign(array(
                    'blog'=>$blog, 
                    'cover'=>$cover, 
