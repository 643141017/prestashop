...
<section id="new-products_block_center{$hook_hash}{$smarty.capture.column_slider}" class="new-products_block_center{$smarty.capture.column_slider} {if !isset($homeverybottom) || !$homeverybottom} block {/if} {if isset($column_slider) && $column_slider} column_block {/if} products_block section {if $hide_mob} hidden-xs {/if} {if $display_as_grid==1} display_as_grid {elseif $display_as_grid==2} display_as_simple {/if}">
	<h3 class="title_block {if (!isset($column_slider) || !$column_slider) && $title_position} title_block_center {/if}"><a href="{$link->getPageLink('new-products')|escape:'html':'UTF-8'}" title="{l s='New products' mod='sthomenew'}">{l s='New products' mod='sthomenew'}</a></h3>
	{if isset($products) AND $products}
    {if !$display_as_grid || ($display_as_grid && isset($column_slider) && $column_slider)}
    <div id="new-itemslider_{$hook_hash}{$smarty.capture.column_slider}" class="new-itemslider{$smarty.capture.column_slider} products_slider block_content">
    ...
    {literal}
    jQuery(function($) { 
        var owl = $("#new-itemslider_{/literal}{$hook_hash}{$smarty.capture.column_slider}{literal} .slides");
        owl.owlCarousel({
            {/literal}
