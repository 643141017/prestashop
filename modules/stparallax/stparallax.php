...
class StParallax extends Module
{
	...
	public function installDb()
	{
        ...
        $return && $return &= (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_parallax_group` (
                ...
                `background_style` tinyint(1) unsigned NOT NULL DEFAULT 0,
                ...
                `play` tinyint(1) unsigned NOT NULL DEFAULT 1, 
                `youtube_loop` tinyint(1) unsigned NOT NULL DEFAULT 1, 
                `youtube_muted` tinyint(1) unsigned NOT NULL DEFAULT 0, 
                `youtube_play` tinyint(1) unsigned NOT NULL DEFAULT 1, 
                `controls` tinyint(1) unsigned NOT NULL DEFAULT 0, 
    ...
    public function sampleData($id_shop)
    {
            ...
            array('id_st_parallax_group' => '', 
                ...
                'child' => array(
                    array(
                        ...
                        'text_color' => '#ffffff', 
                        'width' => '70', 
                        'description' => '<p class="mar_b1 text-center"><em class="icon-quote-left round_icon_wrap"><span class="unvisible">&nbsp;</span></em></p><p class="fs_lg mar_b2">Panda theme is a modern, clean and professional Prestashop theme, it comes with a lot of useful features. Panda theme is fully responsive, it looks stunning on all types of screens and devices.</p><div class="testimonail_author">Max, ST company</div>', 
                    ),
                    array(
                        'text_align' => '2', 
                        'text_color' => '#ffffff', 
                        'width' => '70', 
                        'description' => '<p class="testimonial_starts mar_b2"><em class="icon-star-2 light"><span class="unvisible">&nbsp;</span></em> <em class="icon-star-2 light"><span class="unvisible">&nbsp;</span></em> <em class="icon-star-2 light"><span class="unvisible">&nbsp;</span></em> <em class="icon-star-2"><span class="unvisible">&nbsp;</span></em> <em class="icon-star-2"><span class="unvisible">&nbsp;</span></em></p><p class="fs_lg mar_b2 testimonial_quote">Envato is both a pioneer in creative and web development education, and a one-stop shop for millions of designers needing a range digital assets, from stock multimedia, to web templates, and even snippets of code.</p><div class="testimonail_author">Alexandra, CFO - Panda company</div>', 
                    ),
                ),
            ),
        );
        foreach($samples as $k=>&$sample)
        {
            ...
            $module->bg_pattern = $sample['bg_pattern'];
            ...
            $module->title_color = $sample['title_color'];
            ...
            if($return && $module->id)
            {
                $sample['id_st_parallax_group'] = $module->id;
                Db::getInstance()->insert('st_parallax_group_shop', array(
                    'id_st_parallax_group' => (int)$module->id,
                    'id_shop' => (int)$id_shop,
                ));
            }
        }
        ...
        {
            if(!$sp['id_st_parallax_group'] || !isset($sp['child']) || !count($sp['child']))
                continue;
            foreach($sp['child'] as $k=>$v)
            {
                $module = new StParallaxClass();
                $module->id_st_parallax_group = $sp['id_st_parallax_group'];
                $module->text_align = $v['text_align'];
                $module->text_color = $v['text_color'];
                $module->width = $v['width'];
                $module->active = 1;
                $module->position = $k;
	...
	public function getContent()
	{
        ...
        if(Tools::getValue('act')=='delete_image' && $identi = Tools::getValue('identi'))
        {
            ...
            $slide_group = new StParallaxGroup((int)(int)$identi);
	...
	{        
                ...
                array(
                    ...
                    'values' => array(
                        array(
                            'id' => 'image',
                            'value' => 0,
                            'label' => $this->l('Image(parallax)')),
                        array(
                            'id' => 'hosted',
                            'value' => 1,
                            'label' => $this->l('Hosted video')),
                        /*array(
                            'id' => 'youtube',
                            'value' => 2,
        ...
        $this->fields_form[3]['form'] = array(
            'legend' => array(
                'title' => $this->l('Hosted video'),
                'icon'  => 'icon-cogs'
            ),
            'input' => array( 
                array(
                    'type' => 'text',
                    'label' => $this->l('MP4 format:'),
                    'name' => 'mpfour',
                    'size' => 64,
                    'desc' => $this->l('Example: http://www.yourdomain.com/video.mp4, Internet Explorer 9+, Firefox, Opera, Chrome and Safari support MP4'),
                ),
                array(
                    'type' => 'text',
                    'label' => $this->l('WebM format:'),
                    'name' => 'webm',
                    'size' => 64,
                    'desc' => $this->l('Example: http://www.yourdomain.com/video.webm, Firefox, Chrome and Opera prefer WebM / Ogg formats'),
                ),
                array(
                    'type' => 'text',
                    'label' => $this->l('Ogg format:'),
                    'name' => 'ogg',
                    'size' => 64,
                    'desc' => $this->l('Example: http://www.yourdomain.com/video.ogg, Firefox, Chrome and Opera prefer WebM / Ogg formats'),
                ),
                array(
                    'type' => 'switch',
                    'label' => $this->l('Display Controls:'),
                    'name' => 'controls',
                    'is_bool' => true,
                    'default_value' => 1,
                    'values' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->l('Play video only when in viewport:'),
                    'name' => 'play',
                    'is_bool' => true,
                    'default_value' => 1,
                    'values' => array(
        ...
        $this->fields_form[4]['form'] = array(
            ...
            'input' => array( 
                array(
                    ...
                    'name' => 'youtube',
                    'size' => 64,
                    'desc' => $this->l('Like this: ').'https://www.youtube.com/watch?v=zaK9KEolm00',
                ),
                array(
                    'type' => 'text',
        ...
        if($slide_group->id)
        {
            ...
            if ($slide_group->bg_img)
            {
                ...
                $this->fields_form[2]['form']['input']['bg_img_field']['image'] = '<img width=200 src="'.($slide_group->bg_img).'" /><p><a class="btn btn-default st_delete_image" href="javascript:;" data-id-group="'.(int)$slide_group->id.'"><i class="icon-trash"></i> Delete</a></p>';
            }
        }
