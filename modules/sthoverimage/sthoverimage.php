...
class StHoverImage extends Module
{
	...
	function install()
	{
        $result = Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'image` ADD `hover` BOOLEAN NOT NULL DEFAULT 0;');
        $result &= Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'image_shop` ADD `hover` BOOLEAN NOT NULL DEFAULT 0;');
        if ($result)
        {
	...
	{
        $result =  Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'image` DROP `hover`;');
        $result &=  Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'image_shop` DROP `hover`;');
        foreach(self::$override as $file)
        {
