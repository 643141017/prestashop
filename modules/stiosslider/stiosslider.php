...
class StIosSlider extends Module
{
    ...
    public static $location = array(
        20 => array('id' =>20 , 'name' => 'Full width top', 'templates' => array(0,1,2,3), 'full_width' => 1),
        18 => array('id' =>18 , 'name' => 'Top column', 'templates' => array(0,1,2)),
        ...
        19 => array('id' =>19 , 'name' => 'Bottom column', 'templates' => array(0,1,2)),
        21 => array('id' =>21 , 'name' => 'Full width Bottom', 'templates' => array(0,1,2), 'full_width' => 1),
        7 => array('id' =>7 , 'name' => 'Blog homepage top(fullwidth)', 'templates' => array(0,1,2,3), 'full_width' => 1),
        8 => array('id' =>8 , 'name' => 'Blog homepage top', 'templates' => array(0,1,2)),
        ...
        22 => array('id' =>22 , 'name' => 'Most top of the page', 'templates' => array(3), 'full_width' => 1),
    );
	...
	public function installDb()
	{
		...
		$return &= (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_iosslider_group` (
                ...
                `trans_period` int(10) unsigned NOT NULL DEFAULT 400,
				...
				`prev_next_style` tinyint(1) unsigned NOT NULL DEFAULT 0, 
    ...
    public function sampleData($id_shop)
    {
        ...
        $samples = array(
            array('id_st_iosslider_group' => '', 
                'name' => 'Full with top slideshow',
                'location' => 20, 
                'height' => 600,
                'prev_next' => 4, 
                'pag_nav' => 1, 
                ...
                'child' => array(
                    array(
                        ...
                        'btn_hover_bg' => '#ffffff', 
                        'url' => '', 
                        'description' => '<h2 class="closer" style="font-family:Vollkorn;">CREATIVE THEME</h2><div class="spacer"> </div><p class="center_width_60 color_ccc">Panda theme is a modern, clean and professional Prestashop theme, it comes with a lot of useful features. Panda theme is fully responsive, it looks stunning on all types of screens and devices.</p><div class="spacer"> </div><h4 class="icon_line icon_line_big" style="font-family:Vollkorn;">MODERN &amp; CLEAN</h4><div class="spacer"> </div><div><a class="btn btn-medium last" title="By this theme" href="#">BY THIS THEME</a></div>', 
                    ...
                    array(
                        ...
                        'btn_hover_bg' => '#ffffff', 
                        'url' => '', 
                        'description' => '<h2 class="closer" style="font-family:Vollkorn;">CHECK OUT</h2><h2 class="closer" style="font-family:Vollkorn;">OUR NEW BRANDS</h2><h6>Here you will find our brands that offer the latest in fashion</h6>', 
        ...
        foreach($samples as $k=>&$sample)
        {
            ...
            $module->infiniteSlider = $sample['infiniteSlider'];
            $module->pause = $sample['pause'];
            $module->templates = $sample['templates'];
            ...
            if($return && $module->id)
            {
                $sample['id_st_iosslider_group'] = $module->id;
                Db::getInstance()->insert('st_iosslider_group_shop', array(
                    'id_st_iosslider_group' => (int)$module->id,
                    'id_shop' => (int)$id_shop,
                ));
            }
        }
        ...
        {
            if(!$sp['id_st_iosslider_group'] || !isset($sp['child']) || !count($sp['child']))
                continue;
            foreach($sp['child'] as $k=>$v)
            {
                $module = new StIosSliderClass();
                $module->id_st_iosslider_group = $sp['id_st_iosslider_group'];
                $module->text_position = $v['text_position'];
                $module->text_align = $v['text_align'];
                $module->title_color = $v['title_color'];
                $module->text_width = $v['text_width'];
                $module->text_animation = $v['text_animation'];
                $module->btn_color = $v['btn_color'];
                $module->btn_hover_color = $v['btn_hover_color'];
                $module->btn_hover_bg = $v['btn_hover_bg'];
                $module->active = 1;
                $module->position = $k;
	...
	{
        return Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'st_iosslider`,`'._DB_PREFIX_.'st_iosslider_lang`,`'._DB_PREFIX_.'st_iosslider_font`,`'._DB_PREFIX_.'st_iosslider_group`,`'._DB_PREFIX_.'st_iosslider_group_shop`');
	}
    ...
    private function _checkEnv()
    {
        ...
        if (!is_writeable($file))
            return false;
        $content = 'deny from all'."\r\n".'<Files ~ "(?i)^.*\.(jpg|jpeg|gif|png|bmp|tiff|svg|pdf)$">'."\r\n".'order deny,allow'."\r\n".'allow from all'."\r\n".'</Files>';
    ...
    public function writeCss()
    {
        ...
        if(is_array($options) && count($options))
            foreach($options as $v)    
            {
                ...
                '.$classname.'.style_content a, 
                '.$classname.'.iosslider_btn, 
                '.$classname.'.icon_line{color:'.$v['title_color'].';}
                '.$classname.'.icon_line:after, '.$classname.'.icon_line:before{background-color:'.$v['title_color'].';}
                '.$classname.'.iosslider_btn,
                '.$classname.'.line, '.$classname.'.btn{border-color:'.$v['title_color'].';}';
                if($v['title_bg'])
                    $custom_css .= ''.$classname.'.style_content h1, '.$classname.'.style_content h2, '.$classname.'.style_content h3, '.$classname.'.style_content h4, '.$classname.'.style_content h5, '.$classname.'.style_content h6, '.$classname.'.style_content p{padding: 0.2em 0.3em;background-color:'.$v['title_bg'].';background-color:rgba('.self::hex2rgb($v['title_bg']).',0.4);}'.$classname.'.style_content p{padding: 0.4em 0.6em;}';
        ...
        if(is_array($options) && count($options))
            foreach($options as $v)    
            {
                ...
                if ($v['bg_pattern'] && $v['bg_img']=="")
        			$group_css .= 'background-image: url(../../patterns/'.$v['bg_pattern'].'.png);';
                if ($v['bg_img'])
                ...
                if($group_css)
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.fullwidth_default,#iosSlider_containerOuter_'.$v['id_st_iosslider_group'].'.containerOuter_fullwidth_boxed,#iosSlider_'.$v['id_st_iosslider_group'].'.multi_slide, #iosSlider_containerOuter_'.$v['id_st_iosslider_group'].'.containerOuter_center_background,#iosSlider_containerOuter_'.$v['id_st_iosslider_group'].'.containerOuter_multi_slide{'.$group_css.'}';
                    
                if ($v['prev_next_color'])
        			$custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_prev,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_next{color:'.$v['prev_next_color'].';}';
                if ($v['prev_next_hover'])
        			$custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_prev:hover,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_next:hover{color:'.$v['prev_next_hover'].';}';

                if ($v['prev_next_bg']){
                    $prev_next_bg = self::hex2rgb($v['prev_next_bg'] );
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_prev,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_next{background-color:rgba('.$prev_next_bg.',0.4);}';
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_prev:hover,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_next:hover{background-color:rgba('.$prev_next_bg.',0.8);}';
                }
                /*
                if ($v['prev_next_bg_hover'])
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_prev:hover,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_next:hover{background-color:'.$v['prev_next_bg_hover'].';}';
                */
                if ($v['pag_nav_bg'])
        			$custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_selectors .selectoritem{border-color:'.$v['pag_nav_bg'].';}#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_selectors .selectoritem span{background-color:'.$v['pag_nav_bg'].';}';
                if ($v['pag_nav_bg_active'])
        			$custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_selectors .selectoritem.selected,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_selectors .selectoritem:hover{border-color:'.$v['pag_nav_bg_active'].';}#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_selectors .selectoritem.selected span,#iosSlider_'.$v['id_st_iosslider_group'].' .iosSlider_selectors .selectoritem:hover span{background-color:'.$v['pag_nav_bg_active'].';}';
                    if($v['templates']==2)
                {
                    $width = $v['width'] ? $v['width'] : 900;
                    $height = $v['height'] ? $v['height'] : 500;
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].' .slider .iosSlideritem{width: '.$width.'px;}';
                    $custom_css .= '#iosSlider_containerOuter_'.$v['id_st_iosslider_group'].', #iosSlider_containerOuter_'.$v['id_st_iosslider_group'].' .container_center_background, #iosSlider_containerOuter_'.$v['id_st_iosslider_group'].' .iosSlider_container_center_background,#iosSlider_'.$v['id_st_iosslider_group'].'{height: '.($height+2*$v['padding_tb']).'px;}';
                    
                    $left_right = $width/2+46+$v['slide_padding'];
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.center_background .iosSlider_prev{margin-left:-'.sprintf("%.3f",$left_right+1).'px;}.center_background .iosSlider_next{margin-right:-'.sprintf("%.3f",$left_right-1).'px;}';
                    
                    $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.center_background .slider .iosSlideritem_inner{margin-right:'.(int)$v['slide_padding'].'px;margin-left:'.(int)$v['slide_padding'].'px;}';
                    switch($v['slide_shadow'])
                    {
                        case 1:
                            $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.center_background .slider .iosSlideritem_inner{-webkit-box-shadow: 0 0 6px -3px #000000;-moz-box-shadow: 0 0 6px -3px #000000;box-shadow: 0 0 6px -3px #000000;}';
                        break;
                        case 2:
                            $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.center_background .slider .iosSlideritem_inner{-webkit-box-shadow: 0 0 10px -6px #000000;-moz-box-shadow: 0 0 10px -6px #000000;box-shadow: 0 0 10px -6px #000000;}';
                        break;
                        case 3:
                            $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.center_background .slider .iosSlideritem_inner{-webkit-box-shadow: 0 0 10px -4px #000000);-moz-box-shadow: 0 0 10px -4px #000000;box-shadow: 0 0 10px -4px #000000;}';
                        break;
                        default:
                        break;
                    }
                ...
                elseif($v['templates']==1)
                {
                    ...
                    {
                        case 1:
                            $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.fullwidth_default_boxed .container{-webkit-box-shadow: 0 0 6px -3px #000000;-moz-box-shadow: 0 0 6px -3px #000000;box-shadow: 0 0 6px -3px #000000;}';
                        break;
                        case 2:
                            $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.fullwidth_default_boxed .container{-webkit-box-shadow: 0 0 10px -6px #000000;-moz-box-shadow: 0 0 10px -6px #000000;box-shadow: 0 0 10px -6px #000000;}';
                        break;
                        case 3:
                            $custom_css .= '#iosSlider_'.$v['id_st_iosslider_group'].'.fullwidth_default_boxed .container{-webkit-box-shadow: 0 0 10px -4px #000000);-moz-box-shadow: 0 0 10px -4px #000000;box-shadow: 0 0 10px -4px #000000;}';
                        break;
                        default:
                        break;
                    }
	...
	{        
        ...
        $this->fields_form[1]['form'] = array(
			...
			'input' => array( 
                ...
                array(
                    ...
                    'values' => array(
                        ...
                        array(
                            ...
                            'value' => 4,
                            'label' => $this->l('Display prev/next buttons when mouse hover over, hide them on mobile devices')),
                        array(
                            'id' => 'prev_next_off',
                            'value' => 0,
                            'label' => $this->l('No')),
                ...
                array(
					...
					'values' => array(
						array(
							'id' => 'prev_next_style_full_height',
							'value' => 0,
							'label' => $this->l('Full height')),
						array(
							'id' => 'prev_next_style_rectangle',
							'value' => 1,
							'label' => $this->l('Rectangle')),
						array(
							'id' => 'prev_next_style_circle',
							'value' => 2,
							'label' => $this->l('Circle')),
					),
				),  
                ...
                /*
                2014-11-30 A blank item will occur after the latest item
                array(
					'type' => 'switch',
					'label' => $this->l('Loop:'),
					'name' => 'infiniteSlider',
        ...
        elseif($templates==1){
            ...
            $this->fields_form[0]['form']['input']['slide_height']['label'] = $this->l('Min-height');
            $this->fields_form[0]['form']['input']['slide_height']['required'] = false;
            $this->fields_form[0]['form']['description'] = $this->l('It is highly recommended to enable "Transparent header" option in Theme editor module to make this slideshow works like my demo store 2.');
            unset($this->fields_form[0]['form']['input']['slide_width'],$this->fields_form[0]['form']['input']['slide_width'],$this->fields_form[2],$this->fields_form[3]);
        }
        //reset location
    ...
    public function hookDisplayHeader($params)
    {
		...
		$this->context->controller->addCSS(($this->_path).'views/css/stiosslider.css');
    ...
    public function hookDisplayBottomColumn($params)
    {
        ...
        if (!$this->isCached('stiosslider.tpl', $this->stGetCacheId(19)))
            if(!$this->_prepareHook(19,1))
                return false;
        return $this->display(__FILE__, 'stiosslider.tpl', $this->stGetCacheId(19));
    }
    ...
    public function hookDisplayBanner($params)
    {
        ...
        if (!$this->isCached('stiosslider.tpl', $this->stGetCacheId(22)))
            if(!$this->_prepareHook(22,1))
                return false;
        return $this->display(__FILE__, 'stiosslider.tpl', $this->stGetCacheId(22));
    }
