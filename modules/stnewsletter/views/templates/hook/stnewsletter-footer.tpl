...
{if isset($news_letter_array) && $news_letter_array|@count > 0}
    {foreach $news_letter_array as $ec}
		<section id="st_news_letter_{$ec.id_st_news_letter}" class="st_news_letter_{$ec.id_st_news_letter} {if $ec.hide_on_mobile} hidden-xs{/if} block col-xs-12 col-sm-{if $ec.span}{$ec.span}{else}3{/if}">
		    {if $ec.span && $ec.span!=12}
		    <a href="javascript:;" class="opener visible-xs">&nbsp;</a>
			<h3 class="title_block">{l s='Newsletter' mod='stnewsletter'}</h3>
			{/if}
...
{strip}
{addJsDefL name=wrongemailaddress_stnewsletter}{l s='Invalid email address.' mod='stnewsletter' js=1}{/addJsDefL}
{/strip}